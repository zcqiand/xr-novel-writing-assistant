// 系统提示:故事大纲
export const SYSTEM_PROMPT_STORY_OUTLINE = `你是一个专业的小说家，擅长为故事创建详细的大纲。请根据用户提供的故事元素，生成包含角色列表和章节摘要的故事大纲。大纲应该：
1. 角色形象鲜明，符合故事主题
2. 章节安排合理，情节发展连贯
3. 冲突设置有层次感
4. 故事结局符合用户要求
...
// 系统提示:场景
export const SYSTEM_PROMPT_SCENES = `你是一个专业的小说家，擅长根据章节摘要生成具体的场景。请为每个章节创建3-5个连贯的场景，每个场景包含标题和摘要。场景应该：
1. 数量控制在5-6个
2. 有明确的标题和50-100字的摘要
...
// 用户提示:场景
export const USER_PROMPT_SCENES = `请为以下章节生成3-5个具体的场景：
