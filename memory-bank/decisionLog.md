...
[2025-08-16 12:07:00] - 决策整合生成故事API端点：根据用户需求"整合生成故事的API端点，只保留一个生成故事的API端点"，决定将原有的多个分散API端点（/api/generate-story?stage=outline、/api/generate-story?stage=scenes、/api/generate-story?stage=paragraphs、/api/generate-story?stage=full、/api/generate-story?stage=assemble）整合为一个统一的/api/generate-story端点。实现方案包括：1) 将stage参数改为action参数，支持generate-outline、generate-scenes、generate-paragraphs-bounding、generate-paragraphs、assemble-book五种操作；2) 统一API响应格式，使用success、data、message、error等标准化字段；3) 保持所有生成函数的数据库存储逻辑不变；4) 简化前端调用逻辑，减少API端点管理复杂度。这个整合提高了API的一致性和可维护性，为后续功能扩展提供了更好的架构基础。
[2025-08-16 12:44:00] - 决策修复组装完整书籍数据结构问题：根据用户报告的"组装完整书籍失败: TypeError: outlineData.chapters is not iterable"错误，决定修复数据库字段结构不匹配问题。诊断发现Supabase数据库中的stories表没有直接的chapters字段，章节数据实际存储在outline_data.chapters字段中。修复方案包括：1) 修改assembleFullBook函数的数据访问逻辑，支持outlineData.chapters和outlineData.outline_data.chapters两种数据源；2) 添加运行时数据验证和详细的调试日志；3) 实现优雅的错误处理机制。测试验证显示修复成功，章节数据访问和迭代功能正常工作，解决了数据结构不匹配导致的组装失败问题。
[2025-08-16 13:24:00] - 决策完成组装完整书籍数据结构问题的全面修复：根据用户连续报告的两个错误（TypeError: outlineData.chapters is not iterable 和 TypeError: chapterScenesData[0].scenes_data is not iterable），决定系统化修复数据库字段结构不匹配问题。通过详细的调试过程，发现了两个关键问题：1) stories表没有直接的chapters字段，章节数据存储在outline_data.chapters中；2) story_chapter_scenes表的scenes_data字段结构为{ scenes: [...], chapter: number }而非直接数组。修复方案包括：1) 修改assembleFullBook函数的数据访问逻辑，支持多种数据源；2) 添加运行时数据验证和详细的调试日志；3) 实现优雅的错误处理机制；4) 正确处理嵌套的场景数据结构。测试验证显示两个修复都成功，解决了数据结构不匹配导致的组装失败问题。
[2025-08-16 14:27:30] - 决策实现点击阅读按钮显示完整书籍内容功能：根据用户需求，决定在AI小说创作助手中添加已生成故事的查看功能。实现方案包括：1) 修改主页面（src/app/page.tsx），导入新组件并添加状态管理；2) 实现GeneratedStoriesButton的点击处理逻辑，调用/api/stories/list API获取书籍列表；3) 集成StoriesList组件，传递stories数据和onReadStory回调函数；4) 集成StoryContent组件，传递selectedStoryId和onClose回调函数；5) 实现页面布局逻辑，支持在故事生成界面、书籍列表和书籍内容之间无缝切换；6) 优化状态管理和错误处理，添加滚动位置重置功能；7) 添加CSS动画和样式优化，提升用户体验。这个功能使用户能够查看之前生成的完整故事内容，增强了应用的实用性和用户体验。

[2025-08-17 04:12:00] - 决策实现继续生成功能：根据用户需求"已生成故事清单如果没有完成全部生成任务的故事则显示继续生成按钮，点击后生成没有完成的部分"，决定实现完整的继续生成功能。实现方案包括：1) 分析现有故事生成流程和数据库结构，确定继续生成的技术方案；2) 修改StoriesList组件，添加状态检测逻辑，当故事未完成时显示继续生成按钮；3) 创建新的API端点获取单个故事详情，包括完成状态和生成参数；4) 在统一的生成故事API中添加continue-story action，复用现有的异步生成逻辑；5) 更新前端状态管理，支持继续生成的进度跟踪和错误处理。这个功能大大提升了用户体验，允许用户在中断的故事生成过程中无缝继续。
[2025-08-17 04:44:00] - 添加详细状态更新功能：为继续生成和生成故事API端点添加了总章节数、完成章节数、下一章节总场景数、下一章节完成场景数等详细状态字段。修改了generate-story API的状态更新逻辑，添加了updateStoryStatus辅助函数，同时更新了check-status和单个故事详情API以返回完整的状态信息。

[2025-08-17 14:24:00] - 决策修复数据库约束冲突问题：根据用户报告的"生成任务尚未完成"错误，通过详细日志分析发现根本原因是Supabase数据库中stories表的status字段约束不允许'assemble'状态值。具体修复包括：1) 分析错误日志，识别出code: '23514'约束冲突错误；2) 检查数据库表结构，发现add_status_to_existing_stories_table.sql中的约束缺少'assemble'状态；3) 创建fix_stories_status_constraint.sql脚本，删除旧约束并添加包含'assemble'的新约束；4) 更新原始SQL脚本确保未来部署时包含正确的约束。修复后，异步生成流程中的assemble状态更新将不再失败，解决了生成任务状态不一致的问题。